...
namespace
{
    ...
    constexpr int COVER_RENDER_MIN_WIDTH = 480;
    constexpr int COVER_RENDER_MAX_WIDTH = 4096;
    constexpr double COVER_RENDER_SUPERSAMPLE = 3.0;
...
int CPdfCoverViewCtrl::GetCoverRenderWidth() const
{
    ...
    const int nRenderW = static_cast<int>(nThumbW * dDpiScale * COVER_RENDER_SUPERSAMPLE + 0.5);
    const int nClamped = max(nThumbW, min(nRenderW, COVER_RENDER_MAX_WIDTH));
    return max(COVER_RENDER_MIN_WIDTH, nClamped);
}
void CPdfCoverViewCtrl::ResetCoverContent()
{
    FreeCoverBitmaps();
...
void CPdfCoverViewCtrl::ClearView()
{
    ResetCoverContent();
    m_nSelectedPdfIndex = -1;
    m_arrStoredPdfPaths.RemoveAll();
    m_arrStoredPdfIndices.RemoveAll();
    m_nLastCoverRenderWidth = 0;
    if (GetSafeHwnd() != nullptr)
...
{
    const int nSavedSelection = m_nSelectedPdfIndex;
    m_arrStoredPdfPaths.Copy(arrPdfPaths);
    m_arrStoredPdfIndices.Copy(arrPdfIndices);
    ResetCoverContent();
    const int nCoverRenderWidth = GetCoverRenderWidth();
    m_nLastCoverRenderWidth = nCoverRenderWidth;
    ...
    if (nSavedSelection >= 0)
        m_nSelectedPdfIndex = nSavedSelection;
    if (GetSafeHwnd() != nullptr)
        RedrawWindow(nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
}
...
void CPdfCoverViewCtrl::DrawCoverBitmap(CDC& dc, HBITMAP hCover, const CRect& rcDest)
{
    ...
    if (nImgW > 0 && nImgH > 0)
    {
        const BOOL bOneToOne =
            abs(rcDest.Width() - nImgW) <= 1 && abs(rcDest.Height() - nImgH) <= 1;
        const BOOL bDownscale =
            nImgW >= rcDest.Width() && nImgH >= rcDest.Height();
...
void CPdfCoverViewCtrl::OnSize(UINT nType, int cx, int cy)
{
    ...
    if (GetSafeHwnd() == nullptr)
        return;
    if (!m_arrStoredPdfIndices.IsEmpty())
    {
        const int nNewRenderW = GetCoverRenderWidth();
        const int nThreshold = max(64, m_nLastCoverRenderWidth / 4);
        if (m_nLastCoverRenderWidth <= 0 ||
            abs(nNewRenderW - m_nLastCoverRenderWidth) >= nThreshold)
        {
            ShowPdfCovers(m_arrStoredPdfPaths, m_arrStoredPdfIndices);
            return;
        }
    }
    RedrawWindow(nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
